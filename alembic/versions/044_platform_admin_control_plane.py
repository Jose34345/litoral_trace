...
def upgrade() -> None:
    ...
    # column-scoped SELECT plus matching RLS visibility for this capability.
    op.execute(f"GRANT SELECT (organization_id), DELETE ON TABLE public.us_lacey_operations TO {PLATFORM_ROLE}")
    op.execute("CREATE POLICY us_lacey_operations_platform_select_044 ON public.us_lacey_operations FOR SELECT TO litoral_trace_platform_definer USING (true)")
...
def downgrade() -> None:
    ...
    op.execute("DROP POLICY IF EXISTS us_lacey_operations_platform_select_044 ON public.us_lacey_operations")
    op.execute("DROP POLICY IF EXISTS us_lacey_operations_platform_delete_044 ON public.us_lacey_operations")
    op.execute(f"REVOKE SELECT (organization_id), DELETE ON TABLE public.us_lacey_operations FROM {PLATFORM_ROLE}")
