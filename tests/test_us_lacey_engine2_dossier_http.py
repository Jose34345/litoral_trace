...
def _persist_current(factory, monkeypatch, org, operation):
    ...
    monkeypatch.setattr(engine_module, "process_bundle", lambda **_: bundle_from_resolution(_resolution("dossier.pdf", DocumentType.BILL_OF_LADING, {"bill_of_lading": "MAEU274342495", "country_of_harvest": "CHILE"})))
...
def test_authenticated_get_is_read_only_and_uses_persisted_dossier(engine2_postgres_session_factory, monkeypatch):
    ...
    monkeypatch.setattr(engine_module, "process_bundle", lambda **_: (_ for _ in ()).throw(AssertionError("GET must not execute process_bundle")))
