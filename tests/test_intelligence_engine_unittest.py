...
class TestIntelligenceEngine(unittest.TestCase):
    def test_evaluar_balance_masas_pino_valido(self):
        res = evaluar_balance_masas(volumen_ingresado=500.0, volumen_exportar=225.0, tipo_cultivo="Madera Aserrada (Pino)")
    ...
    def test_evaluar_balance_masas_pino_exceso(self):
        res = evaluar_balance_masas(volumen_ingresado=100.0, volumen_exportar=60.0, tipo_cultivo="Madera Aserrada (Pino)")
    ...
    def test_evaluar_deforestacion_eudr_verde(self):
        ...
        dictamen, obs, base_2020, actual = evaluar_deforestacion_eudr(puntos)
        self.assertEqual(dictamen, "Verde")
        self.assertEqual(base_2020, 0.61)

    def test_evaluar_compliance_lote_completo(self):
        lote = {
        ...
        res = evaluar_compliance_lote(lote, volumen_ingresado_ton=200.0, volumen_exportar_ton=80.0)
        self.assertEqual(res["dictamen"], "Verde")
        self.assertTrue(res["balance_masas"].es_valido)

    def test_generar_dds_json_traces_nt_valid_schema(self):
        lote = {
            "identificador": "RODAL-SUD-04",
            "productor_id": "CUIT-30123456789",
            ...
            "latitud": -27.50,
            "longitud": -58.90,
            "polygon_wkt": "POLYGON((-58.91 -27.51, -58.89 -27.51, -58.89 -27.49, -58.91 -27.49, -58.91 -27.51))"
        }
        json_str = generar_dds_json_traces_nt(lote, volumen_exportar_ton=100.0)
        payload = json.loads(json_str)
        
        self.assertIn("header", payload)
        self.assertIn("declaration", payload)
        self.assertEqual(payload["compliance"]["status"], "COMPLIANT")
        self.assertEqual(payload["declaration"]["commodity"], "Madera Aserrada (Pino)")
        self.assertEqual(payload["declaration"]["volume_tons"], 100.0)

    def test_generar_pdf_reporte_bytes(self):
        lote = {"identificador": "LOTE-DEMO-01", "productor_id": "CUIT-30000000001", "producto_forestal": "Carbón Vegetal"}
        pdf_bytes = generar_pdf_reporte_bytes(lote, "Verde", "Aprobado", 100.0, 20.0, 0.25)
