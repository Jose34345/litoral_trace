...
from litoral_trace.lacey_engine.serialization import BUNDLE_RESOLUTION_SCHEMA_VERSION, serialize_bundle_resolution
...
def _current(factory, monkeypatch):
    ...
    monkeypatch.setattr(service_module, "process_bundle", lambda **values: bundle_from_resolution(resolutions[values["filename"]]))
...
def _view(factory, monkeypatch, org, public_id, *, auto_recover=False):
    monkeypatch.setattr(dossier_module, "engine2_mode", lambda: "SHADOW")
    return UsLaceyEngineDossierService(
...
def test_dossier_reports_failed_only_for_unresolved_current_document(engine2_postgres_session_factory, monkeypatch):
    ...
    session.add(UsLaceyEngineDocumentRun(organization_id=org, operation_id=operation, operation_document_id=link, assurance_document_id=assurance, engine_version=service_module.ENGINE_VERSION, schema_version=BUNDLE_RESOLUTION_SCHEMA_VERSION, source_sha256=sha, role_hint="BILL_OF_LADING", status="FAILED", safe_error_code="TEST", safe_error_message="Test")); session.commit(); session.close()
    monkeypatch.setattr(dossier_module, "engine2_mode", lambda: "SHADOW")
    view = UsLaceyEngineDossierService(session_factory=engine2_postgres_session_factory).get_dossier(organization_id=org, operation_public_id=public_id)
...
def test_dossier_reusable_success_neutralizes_historical_current_failure(engine2_postgres_session_factory, monkeypatch):
    ...
    session = tenant_session(engine2_postgres_session_factory, org); public_id = session.scalar(select(UsLaceyOperation.public_id).where(UsLaceyOperation.id == operation))
    for status, payload in (("FAILED", None), ("SUCCEEDED", serialize_bundle_resolution(bundle_from_resolution(resolution)))): session.add(UsLaceyEngineDocumentRun(organization_id=org, operation_id=operation, operation_document_id=link, assurance_document_id=assurance, engine_version=service_module.ENGINE_VERSION, schema_version=BUNDLE_RESOLUTION_SCHEMA_VERSION, source_sha256=sha, role_hint="BILL_OF_LADING", status=status, resolution_json=payload, safe_error_code="TEST" if status == "FAILED" else None, safe_error_message="Test" if status == "FAILED" else None))
...
def test_dossier_ignores_superseded_document_failure(engine2_postgres_session_factory, monkeypatch):
    ...
    session.add_all((UsLaceyEngineDocumentRun(organization_id=org, operation_id=operation, operation_document_id=old_link, assurance_document_id=old_assurance, engine_version=service_module.ENGINE_VERSION, schema_version=BUNDLE_RESOLUTION_SCHEMA_VERSION, source_sha256=old_sha, role_hint="BILL_OF_LADING", status="FAILED", safe_error_code="TEST", safe_error_message="Test"), UsLaceyEngineDocumentRun(organization_id=org, operation_id=operation, operation_document_id=new_link, assurance_document_id=new_assurance, engine_version=service_module.ENGINE_VERSION, schema_version=BUNDLE_RESOLUTION_SCHEMA_VERSION, source_sha256=new_sha, role_hint="BILL_OF_LADING", status="SUCCEEDED", resolution_json=serialize_bundle_resolution(bundle_from_resolution(_resolution("v2.pdf", DocumentType.BILL_OF_LADING, {"bill_of_lading": "MAEU274342495"}))))))
    session.commit(); session.close()
    view = _view(engine2_postgres_session_factory, monkeypatch, org, public_id)
    assert view.availability is Engine2DossierAvailability.NOT_AVAILABLE and not view.fields
...
def test_dossier_current_shipment_dominates_historical_failures_regardless_of_row_order(engine2_postgres_session_factory, monkeypatch):
    ...
    session = tenant_session(engine2_postgres_session_factory, org); session.add(UsLaceyEngineDocumentRun(organization_id=org, operation_id=operation, operation_document_id=old_link, assurance_document_id=old_assurance, engine_version=service_module.ENGINE_VERSION, schema_version=BUNDLE_RESOLUTION_SCHEMA_VERSION, source_sha256=old_sha, role_hint="COMMERCIAL_INVOICE", status="FAILED", safe_error_code="TEST", safe_error_message="Test")); session.commit(); session.close()
    view = _view(engine2_postgres_session_factory, monkeypatch, org, public_id)
...
def test_dossier_auto_recovers_stale_engine_contract_and_republishes(
    engine2_postgres_session_factory,
    monkeypatch,
):
    factory = engine2_postgres_session_factory
    org, operation, public_id, snapshot = _current(factory, monkeypatch)

    # Simulate a historical snapshot produced by the previous Engine 2 contract.
    ...
    service = UsLaceyEngineDossierService(
        session_factory=factory,
        recovery_service_factory=lambda: UsLaceyEngine2Service(
            session_factory=factory,
            vault_service=FakeVault(b"x"),
        ),
        projection_publisher=lambda **kwargs: published.append(
            (int(kwargs["organization_id"]), int(kwargs["operation_id"]))
        ) or 1,
        auto_recover=True,
    )
    monkeypatch.setattr(dossier_module, "engine2_mode", lambda: "SHADOW")

    view = service.get_dossier(
        organization_id=org,
        operation_public_id=public_id,
    )
    ...
    assert view.fields
    assert published == [(org, operation)]

    session = tenant_session(factory, org)
    runs = (
        session.query(UsLaceyEngineShipmentRun)
        .filter_by(organization_id=org, operation_id=operation)
        .order_by(UsLaceyEngineShipmentRun.id.asc())
        .all()
    )
    assert len(runs) == 2
    assert runs[0].engine_version == "lacey-engine-previous"
    assert runs[1].engine_version == service_module.ENGINE_VERSION
    assert runs[1].source_set_fingerprint != runs[0].source_set_fingerprint
    session.close()
