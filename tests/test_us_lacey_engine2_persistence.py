...
from litoral_trace.lacey_engine.domain import AdmittedCandidate, BundleResolution, DocumentResolution, DocumentType, EvidenceClass, FieldStatus, LayoutBlock, LogicalDocumentResolution, ParsedLayout, Provenance, RawCandidate, ResolvedField
...
def _bundle(resolution: DocumentResolution) -> BundleResolution:
...
def _service(factory, resolutions, **kwargs):
    return UsLaceyEngine2Service(session_factory=factory, vault_service=FakeVault(b"unused"), **kwargs), lambda **values: _bundle(resolutions[values["filename"]])
...
def test_engine2_schema_versions_do_not_reuse_document_or_shipment_caches(engine2_postgres_session_factory, monkeypatch):
    ...
    assert {row.schema_version for row in session.query(UsLaceyEngineDocumentRun).filter_by(operation_document_id=link)} == {"lacey_document_resolution_v0", BUNDLE_RESOLUTION_SCHEMA_VERSION}
...
def test_engine2_engine_and_ruleset_versions_invalidate_independent_caches(engine2_postgres_session_factory, monkeypatch):
    org, operation, _, _, resolutions = _dossier(engine2_postgres_session_factory); monkeypatch.setattr(service_module, "process_bundle", lambda **values: _bundle(resolutions[values["filename"]]))
...
def test_engine2_failed_current_document_blocks_shipment_and_retry_preserves_history(engine2_postgres_session_factory, monkeypatch):
    ...
    def process(**values):
        ...
        return _bundle(resolutions[values["filename"]])
