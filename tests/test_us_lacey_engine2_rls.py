...
from tests.us_lacey_engine2_postgres import FakeVault, bundle_from_resolution, create_test_graph, engine2_postgres_engine, engine2_postgres_session_factory, tenant_session
...
def _seed_snapshot(factory, monkeypatch, content=b"shared"):
    ...
    monkeypatch.setattr(service_module, "process_bundle", lambda **values: bundle_from_resolution(_resolution(values["filename"])))
    ...
    with pytest.raises(DBAPIError):
        with engine2_postgres_engine.begin() as connection:
            ...
            connection.execute(text("""INSERT INTO public.us_lacey_engine_document_runs
                ...
                {"org": a[0], "operation": a[1], "link": a[2], "assurance": a[3], "schema": BUNDLE_RESOLUTION_SCHEMA_VERSION, "sha": a[4]})
...
def test_engine2_composite_tenant_foreign_keys_reject_cross_tenant_rows(engine2_postgres_session_factory, monkeypatch):
    ...
    session.add(UsLaceyEngineDocumentRun(organization_id=b[0], operation_id=a[1], operation_document_id=a[2], assurance_document_id=a[3], engine_version="fk-test", schema_version=BUNDLE_RESOLUTION_SCHEMA_VERSION, source_sha256=a[4], role_hint="BILL_OF_LADING", status="FAILED", safe_error_code="TEST", safe_error_message="Test"))
