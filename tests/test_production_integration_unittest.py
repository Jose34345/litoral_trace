...
class TestFullProductionIntegration(unittest.TestCase):
    def test_e2e_saas_workflow(self):
        ...
        # 2. Análisis histórico orientativo
        lote_dict = {
        ...
        comp_res = evaluar_compliance_lote(
            lote_dict,
            volumen_ingresado_ton=400.0,
            volumen_exportar_ton=160.0,
        )
        self.assertEqual(comp_res["dictamen"], "Verde")
        self.assertEqual(comp_res["profile"], LEGACY_NON_REGULATORY_PROFILE)
        self.assertIsNone(comp_res["regulatory_conclusion"])

        # 3. El generador legacy sólo puede emitir un preview no regulatorio.
        preview_json = generar_dds_json_traces_nt(
            lote_dict,
            volumen_exportar_ton=160.0,
            operador_username=username,
        )
        preview_data = json.loads(preview_json)
        self.assertEqual(preview_data["profile"], LEGACY_NON_REGULATORY_PROFILE)
        self.assertTrue(preview_data["not_a_due_diligence_statement"])
        self.assertFalse(preview_data["submit_ready"])
        self.assertIsNone(preview_data["regulatory_conclusion"])
        self.assertNotIn("compliance", preview_data)
