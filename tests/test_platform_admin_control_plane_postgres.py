...
def test_044_control_plane_authorization_promotion_reset_and_paid_guard() -> None:
    ...
    try:
        with audit.begin() as c:
            ...
            c.execute(text("INSERT INTO public.us_lacey_operation_fields(organization_id,operation_id,merchandise_line_reference,field_name,field_scope) VALUES(:o,:operation_id,'1','cascade-proof','SHIPMENT')"), {"o": tenant_a, "operation_id": operation_a})
            ...
            c.execute(text("INSERT INTO public.us_lacey_operations(public_id,organization_id,created_by_user_id,client_reference,status,document_count,merchandise_line_count) VALUES(:p,:o,:u,:r,'NEW',0,0)"), {"p": str(uuid4()), "o": platform, "u": actor, "r": f"paid-{suffix}"})
        ...
        with audit.connect() as c:
            privileges = c.execute(text("SELECT has_column_privilege('litoral_trace_platform_definer','public.us_lacey_operations','organization_id','SELECT') AS definer_column_select, has_table_privilege('litoral_trace_platform_definer','public.us_lacey_operations','SELECT') AS definer_table_select, has_table_privilege('litoral_trace_platform_definer','public.us_lacey_operations','DELETE') AS definer_delete, has_table_privilege('litoral_trace_app','public.us_lacey_operations','DELETE') AS runtime_delete, has_table_privilege('litoral_trace_us_lacey_worker','public.us_lacey_operations','DELETE') AS worker_delete, has_table_privilege('public','public.us_lacey_operations','DELETE') AS public_delete")).mappings().one()
            assert dict(privileges) == {"definer_column_select": True, "definer_table_select": False, "definer_delete": True, "runtime_delete": False, "worker_delete": False, "public_delete": False}
            assert c.execute(text("SELECT count(*) FROM pg_policies WHERE schemaname='public' AND tablename='us_lacey_operations' AND policyname IN ('us_lacey_operations_platform_select_044','us_lacey_operations_platform_delete_044')")).scalar_one() == 2
