...
def _configure_real_source_set_path(monkeypatch, calls: list[str]) -> SourceSetClaim:
    ...
    monkeypatch.setattr(
        ...
        lambda **_: SimpleNamespace(filename="bom.csv", size_bytes=128, vault_public_id=uuid4()),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda **_: calls.append("project") or SimpleNamespace(projected_count=2, conflict_count=0),
    )
...
def test_worker_builds_product_intelligence_once_for_claimed_source_set(monkeypatch):
    calls: list[str] = []
    claim = _configure_real_source_set_path(monkeypatch, calls)

    monkeypatch.setattr(
        worker,
        "_build_product_intelligence_snapshot",
        lambda **kwargs: calls.append(
            f"pi:{kwargs['claim'].revision_id}:{kwargs['claim'].generation}"
        ),
    )

    result = worker.process_one_us_lacey_job(worker_id="pi-worker")

    assert result.job_status == "COMPLETED"
    assert calls.count(f"pi:{claim.revision_id}:{claim.generation}") == 1
    assert calls.index("canonical") < calls.index(f"pi:{claim.revision_id}:{claim.generation}")
    assert calls.index(f"pi:{claim.revision_id}:{claim.generation}") < calls.index("multilingual")
    assert calls.index("finalize") < calls.index("complete")


def test_product_intelligence_builder_failure_does_not_fail_lacey_job(monkeypatch):
    calls: list[str] = []
    _configure_real_source_set_path(monkeypatch, calls)

    def explode(**_: object):
        calls.append("pi-attempt")
        raise RuntimeError("synthetic Product Intelligence failure")

    monkeypatch.setattr(worker, "build_product_intelligence_snapshot", explode)

    result = worker.process_one_us_lacey_job(worker_id="pi-worker")

    assert result.job_status == "COMPLETED"
    assert "pi-attempt" in calls
