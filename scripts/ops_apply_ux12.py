...
def patch(root: Path) -> None:
    template = root / "src/litoral_trace/templates/traceability_operations.html"
    text = template.read_text(encoding="utf-8")

    replacements = [
        (
            '<section class="space-y-6" aria-labelledby="operations-title">',
            '<section class="min-w-0 space-y-6" aria-labelledby="operations-title">',
            "operations root",
        ),
        (
            '<div class="grid gap-6 xl:grid-cols-3">',
            '<div class="grid min-w-0 gap-6 2xl:grid-cols-3">',
            "operations primary grid",
        ),
        (
            '<section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm xl:col-span-2" aria-labelledby="process-title">',
            '<section class="min-w-0 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm 2xl:col-span-2" aria-labelledby="process-title">',
            "process card",
        ),
        (
            '<div class="grid gap-2 sm:grid-cols-[1fr_10rem]"><select name="input_batch" class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">',
            '<div class="grid min-w-0 gap-2 sm:grid-cols-[minmax(0,1fr)_9rem]"><select name="input_batch" data-batch-picker data-batch-picker-placeholder="{% if row == 0 %}Seleccionar lote{% else %}Entrada adicional{% endif %}" class="min-w-0 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">',
            "process input batch row",
        ),
        (
            '<div class="grid gap-2 lg:grid-cols-[1fr_1.2fr_11rem_7rem_9rem]">',
            '<div class="grid min-w-0 gap-2 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)_9rem_6rem_8rem]">',
            "process output row",
        ),
        (
        ...
        (
            ...
            '<div data-dynamic-batch-rows="shipment" class="mt-3 grid min-w-0 gap-3 md:grid-cols-2 2xl:grid-cols-3">',
            "shipment batch grid",
        ),
        (
            '<div class="grid grid-cols-[1fr_9rem] gap-2"><select name="shipment_batch" class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">',
            '<div data-batch-row class="grid min-w-0 gap-2 rounded-xl border border-slate-200 bg-white p-3 sm:grid-cols-[minmax(0,1fr)_8rem]"><select name="shipment_batch" data-batch-picker data-batch-picker-placeholder="{% if row == 0 %}Seleccionar lote{% else %}Lote adicional{% endif %}" class="min-w-0 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">',
            "shipment batch row",
        ),
        (
            '<input name="shipment_quantity" type="number" min="0.000001" step="0.000001" placeholder="Cantidad" class="rounded-lg border border-slate-300 px-3 py-2 text-sm">',
            '<input name="shipment_quantity" type="number" min="0.000001" step="0.000001" placeholder="Cantidad" class="min-w-0 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">',
            "shipment quantity",
        ),
    ]
    ...
    heading = '<div class="rounded-xl border border-slate-200 bg-slate-50 p-4"><p class="text-xs font-bold uppercase tracking-wide text-slate-600">Lotes del despacho</p>'
    heading_replacement = '<div class="min-w-0 rounded-xl border border-slate-200 bg-slate-50 p-4"><div class="flex flex-wrap items-center justify-between gap-3"><div><p class="text-xs font-bold uppercase tracking-wide text-slate-600">Lotes del despacho</p><p class="mt-1 text-xs text-slate-500">Buscá por código o producto y agregá sólo los lotes que realmente forman parte del envío.</p></div><button type="button" data-add-batch-row="shipment" class="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-slate-400 hover:bg-slate-50"><i class="fa-solid fa-plus" aria-hidden="true"></i>Agregar lote</button></div>'
    text = replace_once(text, heading, heading_replacement, label="shipment lot heading")
    template.write_text(text, encoding="utf-8")

    app = root / "src/litoral_trace/static/src/js/app.js"
    js = app.read_text(encoding="utf-8")
    if "function enhanceBatchPicker(select)" in js:
        raise SystemExit("app.js already contains UX12 batch picker")
...
'''
    ...
    if js.count(anchor) != 1:
        raise SystemExit("app.js boot anchor mismatch")
    js = js.replace(anchor, enhancement + anchor)
    js = replace_once(
        js,
        "  installDisclosureMenus();\n",
        "  installDisclosureMenus();\n  installBatchPickers();\n  installDynamicBatchRows();\n",
        label="boot installers",
    )
    app.write_text(js, encoding="utf-8")

    regression = root / "tests/test_operations_ux12_unittest.py"
