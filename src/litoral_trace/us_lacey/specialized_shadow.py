...
SPECIALIZED_SHADOW_SCHEMA_VERSION = "lacey_multi_agent_shadow_v8"
...
def specialized_engine_version(*, provider: str, model: str, source_set_fingerprint: str | None = None, judge_mode: FieldJudgeMode | str | None = None, projection_mode: SpecializedProjectionMode | str | None = None) -> str:
    ...
    identity = (
        f"v8|{provider}|{model}|schema={SPECIALIZED_SHADOW_SCHEMA_VERSION}|"
    ...
    return f"multi-agent-v8:{hashlib.sha256(identity.encode('utf-8')).hexdigest()[:32]}"
