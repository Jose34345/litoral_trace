...
def evaluate_special_recycled(inputs: SpecialRecycledInput) -> RuleAssessment:
    ...
    if inputs.highly_processed_recycled_material is TriState.NO:
        return _assessment(
            inputs,
            status=RuleStatus.NOT_APPLICABLE,
            reasons=("NOT_RECYCLED_MATERIAL",),
            explanation=(
                "The evidence does not identify this subject as highly processed "
                "recycled plant material."
            ),
        )

    if inputs.highly_processed_recycled_material is TriState.UNKNOWN:
        return _assessment(
            inputs,
            status=RuleStatus.NOT_APPLICABLE,
            reasons=("NO_RECYCLED_MATERIAL_EVIDENCE",),
            explanation=(
                "No evidence identifies this subject as highly processed recycled "
                "plant material, so the SPECIAL / RECYCLED pathway is not applicable."
            ),
        )
