...
class AIProviderConfig:
    ...
    def from_env(cls) -> "AIProviderConfig":
        ...
        try:
            timeout_seconds = max(
                5.0,
                min(float(os.getenv("US_LACEY_AI_TIMEOUT_SECONDS", "90")), 300.0),
            )
        except ValueError:
        ...
        try:
            ...
            max_pages = 8
        allow_external = os.getenv("US_LACEY_AI_ALLOW_EXTERNAL", "0").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
...
def _is_loopback_qwen_target(base_url: str) -> bool:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.hostname:
        return False
    return parsed.hostname.casefold() == "localhost" or parsed.hostname in {
        "127.0.0.1",
        "::1",
    }
