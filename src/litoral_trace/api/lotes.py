...
from litoral_trace.api.batch import (
    ...
    procesar_batch_excel_legacy_endpoint,
...
class LoteCreateRequest(BaseModel):
    ...
    polygon_wkt: str | None = Field(
        default=None,
        description=(
            "Polígono WKT opcional. Si no se informa, "
...
def _identifier_exists(
    ...
    *,
    organization_id: int,
    identificador: str,
    exclude_lote_id: int | None = None,
) -> bool:
    statement = select(Lote.id).where(
        Lote.organization_id == organization_id,
...
# Backward-compatible Python import aliases; HTTP legacy upload is retired.
procesar_batch_excel_endpoint = procesar_batch_excel_legacy_endpoint

# P2.4E: Batch is now an isolated enterprise API router.
