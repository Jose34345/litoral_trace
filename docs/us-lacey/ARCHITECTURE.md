...
P2-02 adds typed adversarial seeds for positional orphan quantities, contradictory botanical identity on one explicit line, and broken one-line arithmetic inference. The scorecard separates `safe_review` from `false_conflict`: a Canonical conflict is a safe review only when the same Shadow entity/field slot already contains more than one distinct supported value. A single-valued Shadow slot that Canonical turns into conflict remains a false conflict. Unresolved component identities are preserved in benchmark ShipmentTruth snapshots so fail-closed exclusion is directly testable without manufacturing line bindings.
