...
| P2-02 adversarial fail-closed benchmark | `tests/lacey_benchmark/test_shadow_canonical_refresh.py` + typed seeds in `adversarial_fixtures.py` + three `pack_adversarial_*.json` baselines | general CI; requires zero false conflicts, explicit safe-review accounting, unresolved-component preservation, and CLI baseline acceptance |
