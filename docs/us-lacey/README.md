...
8. `CLEANUP_CANDIDATES.md` — remaining debt candidates; not permission to delete them
9. `CLEANUP_AUDIT.md` — evidence-backed cleanup decisions and entrypoint classification
...
## Historical material
Historical U.S. Lacey implementation snapshots that are still useful for rationale belong under `docs/archive/us-lacey/`. When an old root path may have inbound links, prefer a small compatibility stub pointing to the archive and back to this directory rather than silently deleting the path.
...
It does not by itself reorganize runtime packages, change API/DB contracts, alter RLS, modify regulatory behavior, change source-set semantics, or authorize deleting runtime files. Those changes require separate evidence-backed work.
