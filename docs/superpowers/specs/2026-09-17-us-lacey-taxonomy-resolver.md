...
Seed records:
1. `Hevea brasiliensis` — accepted species, genus `Hevea`, family context not required by the resolver; authority: Kew Plants of the World Online (POWO), accepted-name record `https://powo.science.kew.org/taxon/349913-1`.
...
3. `rubberwood` / `rubber wood` — curated commercial aliases to `Hevea brasiliensis`; result is `REVIEW_REQUIRED`.
4. `hevea wood` — commercial alias that identifies the genus context but does not prove a species. Return a genus-level `Hevea` candidate with `REVIEW_REQUIRED`, not `Hevea brasiliensis` automatically.
5. `oak` — genus-level `Quercus` candidate with `REVIEW_REQUIRED`.
6. `pine` — genus-level `Pinus` candidate with `REVIEW_REQUIRED`.

`plywood`, generic `wood`, and unknown material names must return `NO_MATCH`.
...
## Testing
Add deterministic unit tests for normalization, accepted-name resolution, synonym/commercial alias review requirements, genus-only safety, no-match behavior, and no fuzzy matching.

Extend Product Intelligence snapshot tests to prove taxonomy enrichment is attached to BOM materials and that `hevea wood` does not become a species automatically.

Full CI and U.S. Lacey PostgreSQL Gate must remain green before the PR can leave draft status.